...
class _ConnectScreenState extends State<ConnectScreen> {
  final _host = TextEditingController(text: 'testepp.nic.kz');
  final _port = TextEditingController(text: '3121');
  final _clId = TextEditingController(text: 'kaznic-test');
  final _pw   = TextEditingController(text: '1234567');
  ...
  Future<void> _connect() async {
    ...
    try {
      final greeting = await _client.connect(_host.text, int.parse(_port.text));
      if (!greeting.isGreeting) { _snack('Нет приветствия от сервера'); return; }

      final login = await _client.login(_clId.text, _pw.text);
      if (!login.ok) { _snack('Ошибка входа: ${login.message}'); return; }
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Column(
          children: [
            Row(children: [
              Expanded(flex: 3, child: TextField(controller: _host, decoration: const InputDecoration(labelText: 'Host'))),
            ...
            TextField(controller: _clId, decoration: const InputDecoration(labelText: 'Login')),
...
// ─── Экран 2: Проверка и просмотр доменов ───────────────────────────────────
...
class _DomainsScreenState extends State<DomainsScreen> {
  ...
  Future<void> _check() async {
    final names = _ctrl.text.split(',').map((s) => s.trim()).where((s) => s.isNotEmpty).toList();
    if (names.isEmpty) return;
    setState(() { _loading = true; _results = null; });
    try {
      final resp = await widget.client.domainCheck(names);
      if (!resp.ok) { _snack('Ошибка: ${resp.message}'); return; }
  ...
  Future<void> _openInfo(String domain) async {
    ...
    try {
      ...
      if (info == null) { _snack('Не удалось разобрать ответ'); return; }
      Navigator.push(context, MaterialPageRoute(builder: (_) => DomainInfoScreen(info: info)));
    } catch (e) {
      _snack('$e');
    } finally {
      if (mounted) setState(() => _loading = false);
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () => Navigator.push(
          context,
          MaterialPageRoute(builder: (_) => CreateDomainScreen(client: widget.client)),
        ),
      ...
      body: Padding(
        ...
        child: Column(
          children: [
            TextField(
              controller: _ctrl,
              decoration: const InputDecoration(labelText: 'Имя домена', hintText: 'example.kz, test.kz'),
            ),
            const SizedBox(height: 12),
            _loading
                ? const CircularProgressIndicator()
                  ...
                  children: [
                    for (final e in _results!.entries)
                      ListTile(
                        ...
                        trailing: e.value
                            ? null
                            : TextButton(onPressed: () => _openInfo(e.key), child: const Text('Подробнее')),
                      ),
                  ],
                ),
              ),
...
// ─── Экран: Детальная информация о домене ────────────────────────────────────
            ...
            Wrap(
              ...
              children: [
                for (final s in info.statuses)
                  Chip(
                    label: Text(s),
                    backgroundColor: s == 'ok' ? Colors.green.shade100 : Colors.orange.shade100,
                  ),
              ],
            ),
          const SizedBox(height: 16),
...
class _CreateDomainScreenState extends State<CreateDomainScreen> {
  ...
  // Фиксированные контакты, созданные заранее на тестовом сервере
  ...
  Future<void> _create() async {
    ...
    if (domain.isEmpty || auth.isEmpty) { _snack('Заполните все поля'); return; }
    ...
    try {
      ...
      setState(() { _success = false; _resultMsg = 'Ошибка: $e'; });
    } finally {
      if (mounted) setState(() => _loading = false);
    }
  }
        ...
        child: Column(
          ...
          children: [
            TextField(
              ...
              autocorrect: false,
              decoration: const InputDecoration(labelText: 'Имя домена', hintText: 'mysite.kz'),
            ),
            const SizedBox(height: 12),
            TextField(
            ...
            const Text('Срок регистрации', style: TextStyle(color: Colors.grey, fontSize: 13)),
            ...
            if (_loading)
              ...
              ElevatedButton(onPressed: _create, child: const Text('Зарегистрировать')),
            if (_resultMsg != null) ...[
              const SizedBox(height: 16),
              Container(
